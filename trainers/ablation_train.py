...
if __name__ == '__main__':
    ...
    p.add_argument('--output_dir',    required=True,
                   help='train results save root directory')
    p.add_argument('--seq_len',       type=int,   help='override observation_seq_len')
    p.add_argument('--p_drop',        type=float, help='override p_drop')
    p.add_argument('--agent_type',    required=True, help='select agent type to train')
    p.add_argument('--num_workers', type=int, help='override train_config num_workers')
    p.add_argument('--transformer_block', choices=['improve', 'original'],
                   help='Transformer block type')
    p.add_argument('--multithreaded', type=int, help='number of worker threads')
    p.add_argument('--curriculum', choices=['none', 'static', 'adaptive'],
    ...
    if args.transformer_block is not None:
        agent_cfg['transformer_block'] = args.transformer_block
    if args.multithreaded is not None:
        agent_cfg['num_workers'] = args.multithreaded
    if args.curriculum is not None:
        train_cfg['curriculum']['mode'] = args.curriculum
        train_cfg['curriculum']['enabled'] = (args.curriculum != 'none')
        agent_cfg['curriculum'] = args.curriculum
    ...
    # Set global random states (for reproducibility)
    ...
    agent_map = {
        ...
        'ablation_base': ('agents.ablation_base_agent', 'AblationITEMATD3Agent'),
    }
    # Agent selection logic
    if args.agent_type.startswith('itemarl_'):
        # ablation experiment agent
        ablation_agent_file = f"{args.agent_type}_agent.py"
        ablation_agent_path = os.path.join(base_dir, "temp_agents", ablation_agent_file)

        if not os.path.exists(ablation_agent_path):
            raise ValueError(f"ablation agent file not exists: {ablation_agent_path}")

        print(f"[Ablation] try to load ablation agent: {ablation_agent_path}")

        # Dynamic import
        spec = importlib.util.spec_from_file_location("ablation_agent", ablation_agent_path)
        ablation_module = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(ablation_module)

        # Find agent class (class name format: ITEMATD3 + type name + Agent)
        expected_class_name = f"ITEMATD3{args.agent_type.replace('itemarl_', '').title().replace('_', '')}Agent"

        print(f"[Ablation] expected class name: {expected_class_name}")

        if hasattr(ablation_module, expected_class_name):
            Agent = getattr(ablation_module, expected_class_name)
            print(f"[Ablation] load ablation agent: {expected_class_name}")
        else:
            # Fallback: find first class starting with ITEMATD3
            print(f"[Ablation] try to find agent class in module: {dir(ablation_module)}")
            for attr_name in dir(ablation_module):
                if attr_name.startswith("ITEMATD3") and attr_name.endswith("Agent"):
                    Agent = getattr(ablation_module, attr_name)
                    print(f"[Ablation] load ablation agent (fallback): {attr_name}")
                    break
            else:
                raise ValueError(f"did not find any class starting with ITEMATD3 and ending with Agent in {ablation_agent_path}")


    elif args.agent_type in agent_map:
        module_path, class_name = agent_map[args.agent_type]
        mod = importlib.import_module(module_path)
        Agent = getattr(mod, class_name)
        print(f"[Train] load predefined agent: {class_name}")
    else:
    ...
    # -----------------------
    # fixed K seeds env_factory (round-robin assign)
    # -----------------------
    K = 8
    # derive K fixed sub-seeds from master_rng (reproducible in same master_seed)
    ...
    # env_factory: each call derives a new sub-seed from master_rng (round-robin assign)
    def make_env_from_factory():
        # thread-safe: get next seed from cycle (round-robin)
